...
class BaseGenerator:
    def __init__(self, d = 3, randomize=False, offset=None, recenter=False, verbose=False):
        """
    ...
    def sample(self, n_samples, reset=False):
        ...
        if self.offset is not None:
            if self.verbose: print("INFO: adding offset: {}".format(self.offset))
            output = (output + self.offset) % 1.
        if self.recenter:
            mean_value = output.mean(axis=0)
            offset_recenter = mean_value - np.ones((self.d,))*0.5
            if self.verbose: print("INFO: recentering: adding offset: {}".format(offset_recenter))
            output = (output - offset_recenter) % 1.
            if self.verbose: print("INFO: recentered: mean: {}".format(output.mean(axis=0)))
    ...
    def sample_gaussian(self, n_samples, loc=None, scale=None, reset=False, trunc_scale=None, sample_buffer = 1.5):
        ...
        arr_output  = - np.sqrt(2.) * scipy.special.erfcinv(2.*arr_unif)
    ...
    def test_gaussian_likelihood(self, n_samples):
        ...
        dist = scipy.stats.multivariate_normal(mean=np.zeros(self.d), cov=np.eye(self.d))
...
class FibonacciLattice(BaseGenerator):
    def __init__(self, d=2, randomize=False, offset=None, recenter=False, verbose=False):
        """
    ...
    def _sample(self, n_samples, reset=False):
        ...
        arr_output = np.stack(
            [
                np.mod(arr_index / self.golden_ratio, 1.),
                (arr_index+0.5) / n_samples
            ],
            axis=-1
        )
        return arr_output 
...
class qmcSampler:
    ...
    def sampleUniform(self, n_samples, resetGenerator = True):
        ...
        samples_uniform = self.generator.sample(
            n_samples
        )
    ...
    def sampleUnitIsotropicGaussian(self, n_samples, truncate = False, truncScale = None, sampleBuffer = 1.25):
        """
        ...
        truncScale: [float] radius to truncate at. Can be interpreted as the number of sigmas to truncate at, since sigma = 1. Default = None.
        ...
        if truncate == True:
            ...
            n_samples_eff = int(
                sampleBuffer*n_samples/ndim_norm_percentile(
                    d=self.d,
                    sigmaRatio=sigma*truncScale
                )
            )
            
            if n_samples/n_samples_eff<.5:
                print(
                    'Efficiency: '+str(np.round(100*n_samples/n_samples_eff,2))+'%'
                )
        ...
        for i in range(n_samples_eff):
            for j in range(self.d):
                samples_gaussian[i,j] = np.sqrt(2)*root(
                    fun = lambda x: .5*(1+erf(x))-samples_uniform[i][j],
                    x0 = 0
                ).x
    ...
    def sampleAnisotropicGaussian(self, n_samples, scales, truncate = False, truncScale = None, sampleBuffer = 1.25):
        """
        ...
        scale: [float] array of length d of each dimensions std.
        ...
        samples_gaussian = self.sampleUnitIsotropicGaussian(n_samples, truncate, truncScale, sampleBuffer)
